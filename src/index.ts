...
export interface StreamingClientOptions {
  ...
  /** Default cap in bytes on forced-raw reads. Overridable per query. */
  readonly filterMaxBytes?: number;
}

/** Parameters for one continuous query. Times are UTC microseconds; `endUs` is exclusive. */
export interface QueryParams {
  ...
  /**
   * Accept decimated output. Defaults to true. When false, forces a raw read (subject to
   * the byte cap) and skips the final resample.
   */
...
/** Parameters for one unit-channel query. Times are UTC microseconds; `endUs` is exclusive. */
...
/** Parameters for a data-availability query over one channel. */
export interface SegmentSpanParams {
 ...
 * client's lifetime. Filter state also lives for the client's lifetime: queries over
 * adjacent windows filter as one continuous signal, and a jump or a gap resets the state
 * per channel. Array reads share an in-flight concurrency cap.
 */
export class StreamingClient {
   ...
   * a montage, or `minMax: false` forces the raw level, since decimated envelopes cannot be
   * filtered or differenced exactly. A forced-raw read that would exceed the byte cap
   * throws {@link FilterWindowTooWide} before any data is fetched. When decimated output is
   * accepted and one pixel spans more than `RESAMPLE_PIXEL_RATIO` source bins, the segment
   * is resampled onto the pixel grid.
   *
   * A window with no overlap yields a segment with empty data. Throws for an unknown
   * channel id, a unit channel, a montage pair whose rates or sample grids differ, or a
   * non-positive `pixelWidthUs`.
   */
  async *query(params: QueryParams): AsyncGenerator<Segment, void, undefined> {
    ...
    if (forceRaw) {
      assertWithinByteCap(
        traces,
        params.filterMaxBytes ?? this.#filterMaxBytes,
      );
    }
   ...
   * for an unknown channel id or a unit channel.
   */
...
/** Throws {@link FilterWindowTooWide} when the planned reads total more than `cap` bytes. */
function assertWithinByteCap(
  ...
  if (requestedBytes > cap) {
    throw new FilterWindowTooWide(requestedBytes, cap);
  }
}
