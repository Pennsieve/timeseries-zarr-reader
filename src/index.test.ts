...
describe("byte cap", () => {
  test("rejects with FilterWindowTooWide before issuing any fetch", async () => {
    ...
    if (!(failure instanceof FilterWindowTooWide)) {
      throw new Error("expected a FilterWindowTooWide rejection");
    }
    expect(failure.requestedBytes).toBe(128);
    expect(failure.maxBytes).toBe(64);
    expect(failure.name).toBe("FilterWindowTooWide");
  });
  ...
  test("applies the cap when minMax is false", async () => {
    const client = makeClient(64);
    await expect(
      collect(
        client.query({
