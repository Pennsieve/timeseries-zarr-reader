...
- Reads of the raw level are capped at 15 MB per query and reject with
  `RawReadTooLargeError` before fetching, whether a filter, a montage, or `raw: true`
  forced the raw level or the pixel width selected it.
